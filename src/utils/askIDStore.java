...
import admin.addEmployeeToStore;
...
/**
 * The <code>askIDItem</code> class represents a window allowing the user to enter an item ID.
 *
 * <p>
 * The window displays a text field for entering the item ID, as well as a "Next" button to validate the entry.
 ...
 *
 * <p>
 * The class can be used for different actions, depending on the parameter passed during initialization.
 * Currently, it is used to delete an item (action 10) or update an item (action 11).
 * </p>
 *
 * <p>
 * Example of usage:
 ...
 *
 * <pre>
 * new askIDItem(10); // To delete an item
 * new askIDItem(11); // To update an item
 * </pre>
 *
 * @see addEmployeeToStore
 * @see updateItem
 * @see connexionBDD
 * @author Tommy Brisset
 * @version 1.0
 */
public class askIDStore extends JFrame {
    ...
    /**
     * Constructor for the `askIDStore` class. Initializes the window to request a store ID.
     *
     * @param action The specified action code (1 for update, 2 for deletion, 3 for another action).
     * @param id_user The ID of the logged-in user.
     */
    public askIDStore(int action,int id_user) {
    ...
    /**
     * Creates and returns the main JPanel of the window for entering the ID.
     *
    ...
    private JPanel getMainJPanel() {
        ...
        infoLabel.setText("ENTER THE ID OF THE STORE : ");
    ...
    /**
     * Adds a vertical separator to the window.
     *
    ...
    void buttonEventHandler() {
        if (isNumberValid.isAValidNumber(idField.getText())) {
            ...
            dispose();
            if (action == 30) {
                new addEmployeeToStore(Integer.parseInt(idField.getText()), id_user);
            }
